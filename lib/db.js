...
import { fmt, fmtT, applySettings } from "./config";
...
export async function loadDb() {
  ...
  // 사업 설정을 먼저 읽어 config 변수에 반영 (차년도 계산 등이 이 값을 쓴다)
  const { data: st } = await sb.from("settings").select("data").eq("id", 1).maybeSingle();
  applySettings(st?.data);
  const CUR_YEAR = cfg.CUR_YEAR;
  const [orgsRaw, profiles, subsRaw, versions, feedbacks, availRaw, confirmsRaw, preRaw, budgetsRaw, docs, reports, alerts, logs, roadmapRaw, expertAvailRaw, expertNotesRaw, appsRaw] =
    await Promise.all([q("orgs"), q("profiles"), q("submissions", "org_id"), q("versions"), q("feedbacks"), q("avail", "date"),
      q("confirms", "date"), q("pre", "org_id"), q("budgets"), q("docs"), q("reports"), q("alerts"), q("logs"), q("roadmap", "m"), q("expert_avail", "date"), q("expert_notes", "expert_id"), q("consult_apps", "at")]);

  const orgs = orgsRaw.map((o) => ({ id: o.id, name: o.name, ini: o.name[0], picked: o.picked, expertId: o.expert_id || null,
    budgetTotal: Number(o.budget_total) || 0, budgetYear: Number(o.budget_year) || 0,
  ...
  // 활성 기관 × 보고서 종류(REPORTS) 마다 제출 상태 한 건
  const orgList = orgs.filter((o) => !o.ended);
  const subs = orgList.flatMap((o) => cfg.REPORTS.map((r) => {
    const s = subsRaw.find((x) => x.org_id === o.id && x.kind === r.key);
    return { id: `${o.id}-${r.key}`, orgId: o.id, kind: r.key, status: s?.status || "none", dueFix: s?.due_fix ? fmt(s.due_fix) : null,
      versions: versions.filter((v) => v.org_id === o.id && v.kind === r.key).sort((a, b) => a.no - b.no)
        .map((v) => ({ no: v.no, at: fmt(v.at), note: v.note, files: v.files, final: v.final })),
      feedbacks: feedbacks.filter((f) => f.org_id === o.id && f.kind === r.key)
        .map((f) => ({ id: f.id, v: f.v, at: fmt(f.at), author: f.author, severity: f.severity, content: f.content, reply: f.reply })) };
  }));

  const confirms = confirmsRaw.map((c) => ({ orgId: c.org_id, type: c.type, date: fmt(c.date), time: c.time || "", zoom: c.zoom, expertId: c.expert_id || null,
  ...
  return {
    orgs, orgList, managers, subs, confirms, profiles, settings: st?.data || {},
    experts: profiles.filter((p) => p.role === "expert").map((p) => ({ id: p.id, name: p.name, title: p.title, ini: p.name[0] || "전" })),
    avail: availRaw.map((a) => ({ orgId: a.org_id, type: a.type, date: fmt(a.date), time: a.time || "", rank: a.rank || null })),
    expertAvail: expertAvailRaw.map((a) => ({ expertId: a.expert_id, type: a.type, date: fmt(a.date), time: a.time || "" })),
    expertNotes: expertNotesRaw.map((n) => ({ expertId: n.expert_id, type: n.type, note: n.note || "", at: fmt(n.at) })),
    // pre[orgId][type] — 회차별 사전 정보
    pre: preRaw.reduce((acc, p) => { (acc[p.org_id] ||= {})[p.type] = { rate: p.rate, progress: p.progress, country: p.country, ask: p.ask, at: fmt(p.at) }; return acc; }, {}),
    consultApps: appsRaw.map((a) => ({ orgId: a.org_id, type: a.type, data: a.data || {}, expertPref: a.expert_pref || "", at: fmt(a.at) })),
    budgets: budgetsRaw.reduce((acc, b) => { (acc[b.org_id] ||= []).push({ id: b.id, round: b.round, date: b.date, docNo: b.doc_no,
      level: b.level, item: b.item, itemTo: b.item_to || "", beforeBasis: b.before_basis, beforeAmt: b.before_amt, afterBasis: b.after_basis, afterAmt: b.after_amt,
    ...
    docs: docs.map((d) => ({ id: d.id, orgId: d.org_id, kind: d.kind, name: d.name, size: d.size, path: d.path, at: fmt(d.at),
      status: d.status, reason: d.reason, amount: Number(d.amount) || 0, by: d.approved_by, approvedAt: d.approved_at ? fmt(d.approved_at) : null, memo: d.memo })),
